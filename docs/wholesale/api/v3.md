...
| Method | Description                                                                                                                                                                                                                                                                                                                                                             |
| :----- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | Returns 1,10 or 100 numbers available for allocation matching the pattern specified. Numbers are returned in price order (cheapest first). One of all, gold, or standard should be specified in the URL:*all* returns all available numbers matching *pattern* **gold** returns only gold numbers matching *pattern* *standard* returns only non-gold numbers matching *pattern*                                               |
...
**Standard Numbers**: Generally available with no setup fee (£0). However, conserved area codes (including Brighton 01273, 020 3, and other Ofcom-conserved ranges) have a setup fee of £2.50 (£5 RRP).

**Gold Numbers**: Start at £2.50 (£5 RRP) for conserved numbers, but premium numbers can cost significantly more. All conserved and desirable ranges (e.g. 020 7, 020 8) are classified as gold.

For detailed pricing information, see: https://simwood.com/2022/08/numbering-pricing/

**Search Strategy**: Since the API returns numbers in price order (cheapest first), you can rely on the first result being the most cost-effective option. For cost-conscious selection, use `available/standard/1/<pattern>` for free numbers where available, or `available/all/1/<pattern>` to see the cheapest option across all categories.
